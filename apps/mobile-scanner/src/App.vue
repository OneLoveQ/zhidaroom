...
const reportLoading = ref(false), liveQuestion = ref<QuestionView | null>(null);
const analysisOpen = ref(false), reportOpen = ref(false), newRunOpen = ref(false);
const progressTimer = ref<number>(), message = ref('请先新建课堂。'), failed = ref('');
...
const currentQuestion = computed(() => session.value?.questions.find((item) => item.id === activeQuestionId.value) ?? liveQuestion.value ?? session.value?.questions.find((item) => item.id === runQuestionIds.value[0]));
...
function applyLiveState(live: Awaited<ReturnType<typeof api.getLiveState>>): void {
  ...
  liveQuestion.value = live.currentQuestion;
